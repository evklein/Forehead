<script lang="ts">
    ...
    function getCumulativeStrokeForHole(holeNumber: number): number {
        console.log(strokes);
        return strokes.filter(s => s.holeNumber === holeNumber).length +
    ...
    function getTotalGir() {
        let totalGir = holes.reduce((total, nextHole) => total + (getStatsForHole(nextHole.holeNumber)?.greenInRegulation ? 1 : 0), 0);
        return totalGir;
    }

    function getTotalGld() {
        let totalGld = holes.reduce((total, nextHole) => total + (getStatsForHole(nextHole.holeNumber)?.greenLightDrive ? 1 : 0), 0);
    ...
    function getTotalScrambling() {
        ...
        let totalScrambling = holes.reduce((total, nextHole) => total + (getStatsForHole(nextHole.holeNumber)?.scrambling ? 1 : 0), 0);
